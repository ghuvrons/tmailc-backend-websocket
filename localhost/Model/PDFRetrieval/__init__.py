...
class PDFRetrieval:
    def __init__(self):
        #self.mysql = mysql
        self.Database_mongo = self.Databases['db2']['companies_data']
    ...
    def retrieve(self, data, sentencesLimit = 3, isExportExcel = False, callback = None):
        ...
        ### query get
        pdf = self.Database_mongo['pdf_report'].aggregate([
            {"$match":_where},
            {"$lookup":{
    ...
    def multiplePdfToTxt(self, category, years = []):
        basePath = self.document_category[category][0]+'/pdf'
    ...
    def uploadPDF(self, ticker, year, pdfPath, category='Annual Report', sic = None, isForceReplace=False):
        ...
        mypdf = self.Database_mongo['pdf_report'].find_one({'_id': data['_id']}, {"path."+self.document_category[category][1]: 1})
        ...
        if isExists:
            ...
            self.Database_mongo['pdf_report'].save(data)
        self.log("\tDone.")
